...
# python test_udp_sha256_2kb.py --suite
# python test_udp_sha256_2kb.py --list-cases 测试集
# python test_udp_sha256_2kb.py --case len_2044_mod256
# python test_udp_sha256_2kb.py --suite
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Send UDP payload with seq_id to FPGA and verify multi-block SHA-256 digest."
    )
...
def build_payload(message: Optional[str], hex_payload: Optional[str]) -> bytes:
    ...
    if len(payload) > MAX_DATA_LEN:
        raise SystemExit(
            ...
            f"Current 2KB protocol supports <= {MAX_DATA_LEN} data bytes "
...
def get_suite_cases() -> List[Tuple[str, bytes]]:
    return [
        ...
        ("len_56_B", b"B" * 56),
        ("len_64_C", b"C" * 64),
        ("len_100_mod26", bytes((ord("a") + (i % 26) for i in range(100)))),
        ("len_512_mod256", bytes((i % 256 for i in range(512)))),
        ("len_1024_mod256", bytes((i % 256 for i in range(1024)))),
        ("len_1468_mod256", bytes((i % 256 for i in range(1468)))),
    ]


def find_case(
    cases: Sequence[Tuple[str, bytes]], label: str
) -> Tuple[str, bytes]:
    for case_label, payload in cases:
        if case_label == label:
            return case_label, payload
    raise SystemExit(f"Unknown --case '{label}'. Use --list-cases to inspect names.")
...
def run_repeat(args: argparse.Namespace, payload: bytes) -> int:
    pass_count = 0
    label = getattr(args, "selected_case_label", None)
...
def print_cases(cases: Iterable[Tuple[str, bytes]]) -> None:
    for label, payload in cases:
        print(f"{label:16} len={len(payload):4d} hex={format_payload_preview(payload)}")
...
def main() -> int:
    ...
    if args.case is not None:
        label, payload = find_case(cases, args.case)
        args.selected_case_label = label
        args.repeat = 1
